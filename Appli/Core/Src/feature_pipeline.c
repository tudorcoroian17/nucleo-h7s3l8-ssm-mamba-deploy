...
static float32_t frame_history[MEL_N_FFT]; /* zero-initialized: static storage duration */
...
void FeaturePipeline_ComputeLogMelFrame(const float32_t *frame,
		float32_t *logmel_out) {
	/*	Window: elementwise multiply by the periodic Hann window that
	 *  matches librosa.stft(window='hann'). */
	arm_mult_f32((float32_t*) frame, (float32_t*) hann_window, windowed_frame,
			MEL_N_FFT);
...
void FeaturePipeline_PushHop(const float32_t *new_hop_samples) {
	memmove(frame_history, frame_history + MEL_HOP_LENGTH,
			(MEL_N_FFT - MEL_HOP_LENGTH) * sizeof(float32_t));
	memcpy(frame_history + (MEL_N_FFT - MEL_HOP_LENGTH), new_hop_samples,
			MEL_HOP_LENGTH * sizeof(float32_t));
}
