...
void SystemCoreClockUpdate(void) {
	...
	case 0x18: /* PLL1 used as system clock  source */
		...
		if ((RCC->PLLCFGR & RCC_PLLCFGR_PLL1FRACEN) != 0U) {
			pllfracn = (float_t) (uint32_t) (((RCC->PLL1FRACR
					& RCC_PLL1FRACR_FRACN) >> RCC_PLL1FRACR_FRACN_Pos));
		} else {
			pllfracn = (float_t) 0U;
		}
		...
		if (pllm != 0U) {
			...
			case 0x02: /* HSE used as PLL1 clock source */
				pllvco = ((float_t) HSE_VALUE / (float_t) pllm)
						* ((float_t) (uint32_t) (RCC->PLL1DIVR1
								& RCC_PLL1DIVR1_DIVN)
			...
			default:
				hsivalue = (HSI_VALUE
						>> ((RCC->CR & RCC_CR_HSIDIV) >> RCC_CR_HSIDIV_Pos));
